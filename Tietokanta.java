...
public class Tietokanta {
  ...
  private static void listaaTopEnitenPisteitaOpettajat(Connection c) {
    ...
    try {
      ...
      PreparedStatement p = c.prepareStatement(
          "SELECT O.nimi nimi, SUM(K.laajuus) opMaara FROM Opettajat O, Kurssit K, Suoritukset S WHERE K.id=S.kurssi_id AND O.id=K.opettaja_id GROUP BY O.id ORDER BY opMaara DESC LIMIT ?;");
      p.setString(1, "" + top);

      ResultSet r = p.executeQuery();
      System.out.println("opettaja \t\top");
      while (r.next()) {
        if (r.getString("nimi").length() < 16)
          System.out.println(r.getString("nimi") + "\t\t" + r.getInt("opMaara"));
        else
          System.out.println(r.getString("nimi") + "\t" + r.getInt("opMaara"));
  ...
  private static void laskeKurssinKeskiarvo(Connection c) throws SQLException {
    ...
    PreparedStatement p = c.prepareStatement(
        "SELECT AVG(S.arvosana) keskiarvo, COUNT(*) määrä FROM Kurssit K, Suoritukset S WHERE K.id=S.kurssi_id AND LOWER(K.nimi)=?");
    p.setString(1, nimi);
  ...
  private static void haeOpiskelijanSuoritukset(Connection c) throws SQLException {
    ...
    PreparedStatement p = c.prepareStatement(
        "SELECT K.nimi kurssi, K.laajuus op, S.paivays päiväys, S.arvosana arvosana FROM Kurssit K, Suoritukset S, Opiskelijat O WHERE K.id=S.kurssi_id AND O.id=S.opiskelija_id AND O.nimi=? ORDER BY päiväys");
    p.setString(1, nimi);
    ...
    if (r.next()) {
      ...
      System.out.println();
      while (r.next()) {
        System.out.format("%7s %2d %10s %2d", r.getString("kurssi"), r.getInt("op"), r.getString("päiväys"),
            r.getInt("arvosana"));
        System.out.println();
      }
    } else {
  ...
  private static void haeVuodenOpintopisteet(Connection c) throws SQLException {
    ...
    PreparedStatement p = c.prepareStatement(
        "SELECT SUM(K.laajuus) opintopisteet FROM Kurssit K, Suoritukset S WHERE K.id=S.kurssi_id AND S.paivays LIKE ?");
    p.setString(1, vuosi);
